            ...
            for j=1:length(Data(1).APbinID)
                if Data(i).APDivision(nc,j)
                    ...
                    if (j>=jMin)&(j<=jMax)
                        ...
                        % As the APDivision.mat is not defined well in
                        % CheckDivisionTimes.m step
                        if nc==14
                           %FrameRange=Data(i).APDivision(nc,j):length(Data(i).ElapsedTime);
                           FrameRange = Data(i).nc14:length(Data(i).ElapsedTime);
                        elseif nc==13
                           FrameRange = Data(i).nc13:Data(i).nc14;
                        elseif nc==12
                           FrameRange = Data(i).nc12:Data(i).nc13;
                        else
