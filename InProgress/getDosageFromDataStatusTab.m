function [Dosage,DosageText] = getDosageFromDataStatusTab(dataTypeTabContents)
...
% Locate the RowName cells within the tab
DosageRow = find(strcmpi(dataTypeTabContents(:,1),RowName));
DosageColumns = find( contains(...
    strrep( dataTypeTabContents(prefixRow,:) , ' ', ''), 'x', 'IgnoreCase', true) ); 
                 
if isempty(DosageRow) || isempty(DosageColumns)
    error('Unable to find Prefixes in DataStatus.xslx tab. Could be missing or incorrectly formatted.')
end
                 
DosageText = dataTypeTabContents(DosageRow,DosageColumns);
