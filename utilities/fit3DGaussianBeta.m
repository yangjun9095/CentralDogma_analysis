...
% totalFluo: Estimate of total fluorescence using fit parameters

function [GaussFit, totalFluo] = fit3DGaussianBeta(snip3D,varargin)
...
ub_vec = double([max(snip3D(:)),yDim,xDim,zDim,yDim,xDim,zDim,1,1,1,median(snip3D(:))]);
...
% total fluorescence
totalFluo = GaussFit(1) * sqrt((2*pi)^3 * det(cov_mat(GaussFit)));
