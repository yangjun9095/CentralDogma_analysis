...
function temp_gillespie = synthetic_rate_gillespie_temporal(seq_length,...
            alpha, w, K, k_on_lower,k_on_upper,k_off_lower,k_off_upper, deltaT, r_emission, noise, ...
...
R_array = zeros(K,K,length(t_ref));
if K == 3   
    % from 0
...
elseif K == 2        
    % from 0
    R_array(1,1,:) = -k_on_ref;
    R_array(2,1,:) = k_on_ref;    
    % from 1
    R_array(1,2,:) = k_off_ref;
    R_array(2,2,:) = -k_off_ref;
else
    error('Only 2 and 3 state simulations supported in current version')
end
%%% Simulations
jump_times = [0];
promoter_states = [randsample(1:K,1,true,pi0)];
