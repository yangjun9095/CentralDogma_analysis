function gillespie = synthetic_rate_gillespie_semi_poisson(seq_length, kappa, ...
    ...
    % arrays for storing the naive states and times of transitions 
    % during the Gillespie simulation
    naive_states = [];
    p_times = [];
    ...
    % ------------- Simulate a sequence of promoter states ----------------
    ...
    state_ind = randsample(1:length(state_id_vec), 1, true, ...
        (state_id_vec==state_id)/sum(state_id_vec==state_id));
    ...
    for i = 1:length(names)
        ...
        % if instantaneous initiation assumed, adjust the mRNA calibration
        % accordingly
        if instant_fluo(i) == 1
        ...
        % find the fluorescence at the uniformly distributed points taking 
        % the MS2 loops into account
        ...
        for k = 1:seq_length
            arrivals = arrival_times(1==((arrival_times >= (k-1)*deltaT)...
                                          .*(arrival_times < k*deltaT)));
    ...
    for k = 1:seq_length
        ...
        for i = 1:(length(naive_window)-1)
            ...
            fluo_MS2_orig(k) = fluo_MS2_orig(k) + r_emission_orig(naive_window(i)) * ...
                          ms2_loading_coeff_integral(kappa, w, deltaT, t1, t2);
        end
    ...
    for i = 1:length(names)        
        ...
        fluo_trajectories(i).fluo_per_rna_effective = fluo_trajectories(i).fluo_per_rna * factor;
        if adjust_means == 1
